  ...
  try {
    const [rows] = await pool.query(
      `SELECT b.id, b.name_bn, b.name_en, b.slug, b.sort_order,
              ...
              COALESCE(
                NULLIF(TRIM(b.cover_image), ''),
                (SELECT p2.image_url FROM products p2
                 WHERE p2.brand_id = b.id AND p2.is_active = 1
                   AND p2.image_url IS NOT NULL AND TRIM(p2.image_url) <> ''
                 ORDER BY p2.id ASC LIMIT 1)
              ) AS cover_image
       FROM brands b
       LEFT JOIN products p ON p.brand_id = b.id AND p.is_active = 1
       GROUP BY b.id, b.name_bn, b.name_en, b.slug, b.sort_order, b.cover_image
